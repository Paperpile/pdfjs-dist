...
var _initialization = require("../../scripting_api/initialization.js");
...
describe("Util", function () {
  ...
  beforeAll(function (done) {
    ...
    (0, _initialization.initSandbox)(data, extra, sandbox);
    util = sandbox.util;
    done();
  });
  afterAll(function () {
  ...
  describe("printd", function () {
    it("should print a date according to a format", function (done) {
      var date = new Date("April 15, 1707 3:14:15");
  ...
  describe("scand", function () {
    it("should parse a date according to a format", function (done) {
      ...
      expect(util.scand(2, "4/15/07 3:14:15 am")).toEqual(new Date("April 15, 2007 3:14:15"));
      done();
    });
  });
  describe("printf", function () {
    it("should print some data according to a format", function (done) {
      expect(util.printf("Integer numbers: %d, %d,...", 1.234, 56.789)).toEqual("Integer numbers: 1, 56,...");
      expect(util.printf("Hex numbers: %x, %x,...", 1234, 56789)).toEqual("Hex numbers: 4D2, DDD5,...");
      expect(util.printf("Hex numbers with 0x: %#x, %#x,...", 1234, 56789)).toEqual("Hex numbers with 0x: 0x4D2, 0xDDD5,...");
      expect(util.printf("Decimal number: %,0+.3f", 1234567.89123)).toEqual("Decimal number: +1,234,567.891");
      expect(util.printf("Decimal number: %,0+8.3f", 1.234567)).toEqual("Decimal number: +  1.235");
      done();
    });
  ...
  describe("printx", function () {
    it("should print some data according to a format", function (done) {
      expect(util.printx("9 (999) 999-9999", "aaa14159697489zzz")).toEqual("1 (415) 969-7489");
      done();
    });
  });
});