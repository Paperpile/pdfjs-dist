  ...
  describe("splitCombinedOperations", function () {
    it("should reject unknown operations", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        ...
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.fill);
    ...
    it("should handle one operation", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        expect(!!result.fnArray && !!result.argsArray).toEqual(true);
        expect(result.fnArray.length).toEqual(1);
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.restore);
        done();
      });
    });
    it("should handle two glued operations", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, resources, function (result) {
        ...
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.dependency);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.paintImageXObject);
        expect(result.fnArray[2]).toEqual(_util_metapdf.OPS.restore);
    ...
    it("should handle three glued operations", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        ...
        expect(result.fnArray.length).toEqual(3);
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.fill);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.fill);
        expect(result.fnArray[2]).toEqual(_util_metapdf.OPS.fill);
        done();
      });
    });
    it("should handle three glued operations #2", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, resources, function (result) {
        expect(!!result.fnArray && !!result.argsArray).toEqual(true);
        expect(result.fnArray.length).toEqual(3);
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.eoFillStroke);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.fillStroke);
        expect(result.fnArray[2]).toEqual(_util_metapdf.OPS.eoFill);
        done();
      });
    });
    it("should handle glued operations and operands", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        ...
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.fill);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.setTextRise);
    ...
    it("should handle glued operations and literals", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        expect(!!result.fnArray && !!result.argsArray).toEqual(true);
        expect(result.fnArray.length).toEqual(3);
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.setFlatness);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.setRenderingIntent);
        expect(result.fnArray[2]).toEqual(_util_metapdf.OPS.endPath);
        expect(result.argsArray.length).toEqual(3);
  ...
  describe("validateNumberOfArgs", function () {
    it("should execute if correct number of arguments", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        ...
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.setCharWidth);
        done();
      });
    });
    ...
    it("should execute if nested commands", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, resources, function (result) {
        expect(result.fnArray.length).toEqual(3);
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.setGState);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.dependency);
        expect(result.fnArray[2]).toEqual(_util_metapdf.OPS.setFont);
        expect(result.argsArray.length).toEqual(3);
    ...
    it("should error if (many) path operators have too few arguments " + "(bug 1443140)", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, lineToStream, new ResourcesMock(), function (error) {
        expect(error instanceof _util_metapdf.FormatError).toEqual(true);
    ...
    it("should close opened saves", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        ...
        expect(result.fnArray[0]).toEqual(_util_metapdf.OPS.save);
        expect(result.fnArray[1]).toEqual(_util_metapdf.OPS.save);
        expect(result.fnArray[2]).toEqual(_util_metapdf.OPS.restore);
        expect(result.fnArray[3]).toEqual(_util_metapdf.OPS.restore);
        done();
      });
    });
    it("should error on paintXObject if name is missing", function (done) {
      ...
      runOperatorListCheck(partialEvaluator, stream, new ResourcesMock(), function (result) {
        expect(result instanceof _util_metapdf.FormatError).toEqual(true);
