...
(function listenFindEvents() {
  ...
  var handleEvent = function handleEvent(_ref) {
    ...
    if (!_app_metapdf.PDFViewerApplication.initialized) {
      return;
    }

    if (type === "findbarclose") {
      _app_metapdf.PDFViewerApplication.eventBus.dispatch(type, {
        source: window
      });
    ...
    _app_metapdf.PDFViewerApplication.eventBus.dispatch("find", {
      source: window,
...
(function listenZoomEvents() {
  ...
  var handleEvent = function handleEvent(_ref2) {
    ...
    if (type === "zoomreset" && _app_metapdf.PDFViewerApplication.pdfViewer.currentScaleValue === _ui_utils_metapdf.DEFAULT_SCALE_VALUE) {
      return;
    }
...
}(_app_metapdf.DefaultExternalServices);

_app_metapdf.PDFViewerApplication.externalServices = FirefoxExternalServices;
