...
var _pdf_thumbnail_view_metapdf = require("./pdf_thumbnail_view_metapdf.js");
...
var PDFThumbnailViewer = /*#__PURE__*/function () {
  function PDFThumbnailViewer(_ref) {
    ...
    this.l10n = l10n;
    this.scroll = (0, _ui_utils_metapdf.watchScroll)(this.container, this._scrollUpdated.bind(this));
  ...
  _createClass(PDFThumbnailViewer, [{
    ...
    value: function scrollThumbnailIntoView(pageNumber) {
      ...
      var selected = document.querySelector('.mp-thumbnail.selected');

      if (selected) {
        selected.classList.remove('mp-selected');
      }

      var thumbnail = document.querySelector('div.mp-thumbnail[data-page-number="' + pageNumber + '"]');

      if (thumbnail) {
        thumbnail.classList.add('mp-selected');
      }
      ...
      if (numVisibleThumbs > 0) {
        ...
        if (shouldScroll) {
          (0, _ui_utils_metapdf.scrollIntoView)(thumbnailView.div, {
    ...
    value: function cleanup() {
      _pdf_thumbnail_view_metapdf.PDFThumbnailView.cleanup();
    }
  }, {
    key: "_resetView",
    ...
    value: function setDocument(pdfDocument) {
      ...
      firstPagePromise.then(function (firstPdfPage) {
        ...
        for (var pageNum = 1; pageNum <= pagesCount; ++pageNum) {
          var thumbnail = new _pdf_thumbnail_view_metapdf.PDFThumbnailView({
    ...
    value: function forceRendering() {
      ...
      if (visibleThumbs.first) {
        var updateRange = [];

        for (var i = visibleThumbs.first.id; i <= visibleThumbs.last.id; i++) {
          updateRange.push(i);
        }

        MP.actions.Viewer.updateThumbnails(updateRange);
        return;
      }
