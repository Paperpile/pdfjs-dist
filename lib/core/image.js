...
var PDFImage = /*#__PURE__*/function () {
  function PDFImage(_ref) {
    ...
    if (Number.isInteger(image.width) && image.width > 0 && Number.isInteger(image.height) && image.height > 0 && (image.width !== width || image.height !== height)) {
      (0, _util_metapdf.warn)("PDFImage - using the Width/Height of the image data, " + "rather than the image dictionary.");
      width = image.width;
      height = image.height;
    }

    if (width < 1 || height < 1) {
      throw new _util_metapdf.FormatError("Invalid image width: ".concat(width, " or height: ").concat(height));
    }
    ...
    if (!bitsPerComponent) {
      ...
      if (!bitsPerComponent) {
        if (this.imageMask) {
          ...
          throw new _util_metapdf.FormatError("Bits per component missing in image: ".concat(this.imageMask));
        }
      }
    }
    ...
    if (!this.imageMask) {
      ...
      if (!colorSpace) {
        (0, _util_metapdf.info)("JPX images (which do not require color spaces)");
      ...
      this.colorSpace = _colorspace_metapdf.ColorSpace.parse({
        cs: colorSpace,
        xref: xref,
    ...
    if (this.decode && (this.colorSpace && !this.colorSpace.isDefaultDecode(this.decode, bitsPerComponent) || isMask && !_colorspace_metapdf.ColorSpace.isDefaultDecode(this.decode, 1))) {
    ...
    if (smask) {
      ...
      if ((0, _primitives.isStream)(mask)) {
        ...
        if (!imageMask) {
          (0, _util_metapdf.warn)("Ignoring /Mask in image without /ImageMask.");
        } else {
  ...
  _createClass(PDFImage, [{
    ...
    value: function fillOpacity(rgbaBuf, width, height, actualHeight, image) {
      ...
      if (smask) {
        ...
        if (mask instanceof PDFImage) {
          ...
          throw new _util_metapdf.FormatError("Unknown mask format.");
        }
      }
    ...
    value: function createImageData() {
      ...
      if (!forceRGBA) {
        ...
        if (this.colorSpace.name === "DeviceGray" && bpc === 1) {
          kind = _util_metapdf.ImageKind.GRAYSCALE_1BPP;
        } else if (this.colorSpace.name === "DeviceRGB" && bpc === 8 && !this.needsDecode) {
          kind = _util_metapdf.ImageKind.RGB_24BPP;
        }
        ...
        if (kind && !this.smask && !this.mask && drawWidth === originalWidth && drawHeight === originalHeight) {
          ...
          if (this.needsDecode) {
            (0, _util_metapdf.assert)(kind === _util_metapdf.ImageKind.GRAYSCALE_1BPP, "PDFImage.createImageData: The image must be grayscale.");
        ...
        if (this.image instanceof _jpeg_stream.JpegStream && !this.smask && !this.mask) {
          ...
          switch (this.colorSpace.name) {
            ...
            case "DeviceRGB":
            case "DeviceCMYK":
              imgData.kind = _util_metapdf.ImageKind.RGB_24BPP;
      ...
      if (!forceRGBA && !this.smask && !this.mask) {
        ...
        imgData.kind = _util_metapdf.ImageKind.RGBA_32BPP;
    ...
    value: function fillGrayBuffer(buffer) {
      ...
      if (numComps !== 1) {
        throw new _util_metapdf.FormatError("Reading gray scale from a color image: ".concat(numComps));
      }
              ...
              case 0:
                ...
                if (smask) {
                  ...
                  if ((0, _primitives.isStream)(mask) || Array.isArray(mask)) {
                    ...
                    (0, _util_metapdf.warn)("Unsupported mask format.");
                  }
                }
